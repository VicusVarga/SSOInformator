...
namespace SSOInformator
{
    ...
    {
        private static string ssoInformatorPath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments), "SSOInformator");
        private string settingsPath = Path.Combine(ssoInformatorPath, "Settings.ini");
        ...
        {
            InitializeComponent();
            if (!Directory.Exists(ssoInformatorPath))
            {
                Directory.CreateDirectory(ssoInformatorPath);
            }
        ...
        private void OKButton_Click(object sender, RoutedEventArgs e) // Кнопка "ок"
        {
            ...
            if (settings != null && values != null && settings.Length == 2 && values.Length == 3 && settings[0] == delayValue && values[0] == ipValue && values[1] == loginValue && values[2] == passwordValue) //Если пользователь не изменил настройки, но нажал "ок" просто закрыть окно без перезапуска подключения.
            {
